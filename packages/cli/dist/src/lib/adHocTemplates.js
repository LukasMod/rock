...
function createAdHocTemplate({ platform, appName, version, identifier, icon, buttonText, toastMessage, script, instructions, adhocDescription, }) {
    return `<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
...
export function templateIndexHtmlIOS({ appName, version, bundleIdentifier, }) {
    ...
    `,
        adhocDescription: "This app is distributed via ad-hoc distribution for testing purposes. Your device must either be enrolled in enterprise distribution or have its UDID added to the app's provisioning profile.",
    });
}
...
export function templateIndexHtmlAndroid({ appName, version, packageName, }) {
    return createAdHocTemplate({
        platform: 'Android',
        appName,
        version,
    ...
    `,
        adhocDescription: 'This app is distributed via ad-hoc distribution for testing purposes. You may need to enable installation from unknown sources in your Android device settings.',
    });
}
